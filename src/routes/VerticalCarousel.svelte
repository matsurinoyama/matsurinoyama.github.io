<script>
  ...
  import Carousel_Image from "./VerticalCarousel_Image.svelte";
  ...
  async function getElementHeight() {
    if (carouselTrack_ID && !isImagesLoaded) {
      ...
      totalHeight = carouselTrack_ID.getBoundingClientRect().height / 2;
  ...
  onMount(getElementHeight);
  afterUpdate(getElementHeight);
  ...
  if (
    ...
    carouselAnim_Key2 = "translateY(0px);";
    carouselAnim_Key1 = "translateY(calc(var(--total-length)));";
  } else {
    carouselAnim_Key1 = "translateY(0px);";
    carouselAnim_Key2 = "translateY(calc(var(--total-length)));";
  }
...
<div class="carouselContainer">
  <div
    ...
    style={`--total-length: -${totalHeight}px; --total-num: ${totalNum}; --anim-time: ${carouselAnim_Duration}s; --anim-key1: ${carouselAnim_Key1}; --anim-key2: ${carouselAnim_Key2};`}
  >
